...
class LdaTest extends \PHPUnit_Framework_Testcase {
	...
	public function testLda() {
		$lda = new Lda ( new DataAsFeatures (), // feature factory
10, // number of topics
1, // dirichlet prior per doc topic dist
1 ) // dirichlet prior per word topic dist
;
		
		$this->assertInstanceOf ( "NlpTools\Models\Lda", $lda );
		
		$docs = $lda->generateDocs ( $this->tset );
		$this->assertCount ( count ( $this->tset ), $docs );
		...
		for($i = 0; $i < 100; $i ++) {
			...
			foreach ( $topics as $t => $topic ) {
				...
				$max = max ( $topic );
				$this->createImage ( array_map ( function ($x) use($topic, $max) {
	...
	protected function createTopics() {
		...
		$this->topics = array_map ( function ($topic) {
			...
			return array_map ( function ($ti) use($s) {
				return $ti / $s;
			}, $t );
		}, $topics );
	...
	protected function loadData() {
		...
		foreach ( new \DirectoryIterator ( "{$this->path}/data" ) as $f ) {
			...
			$this->tset->addDocument ( "", new TokensDocument ( $this->fromImg ( $f->getRealPath () ) ) );
		}
	}
