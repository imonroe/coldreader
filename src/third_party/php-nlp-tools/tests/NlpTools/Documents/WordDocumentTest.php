...
class WordDocumentTest extends \PHPUnit_Framework_TestCase {
	...
	public function testTokenSelection() {
		foreach ( $this->tokens as $i => $t ) {
			...
			$this->assertEquals ( $t, $w, "The {$i}th token should be $t not $w" );
			
			// no context means prev,next are empty
			$this->assertCount ( 0, $prev );
	 ...
	 * until it reaches the edges of the token list.
	 * Check the
	 * previous tokens.
	 */
	public function testPrevContext() {
		for($i = 0; $i < 5; $i ++) {
			...
			$this->assertCount ( $i, $prev, "With $i words context prev should be $i words long" );
			for($j = 3, $y = $i - 1; $j >= 4 - $i; $y --, $j --) {
				$this->assertEquals ( $this->tokens [$j], $prev [$y] );
			}
		}
	}
	...
	public function testNextContext() {
		for($i = 0; $i < 5; $i ++) {
			...
			$this->assertCount ( $i, $next, "With $i words context next should be $i words long" );
			for($j = 5; $j < 5 + $i; $j ++) {
				$this->assertEquals ( $this->tokens [$j], $next [$j - 5] );
			}
		}
	}
}
