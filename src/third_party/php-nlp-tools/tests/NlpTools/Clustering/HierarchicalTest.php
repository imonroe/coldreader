...
class HierarchicalTest extends ClusteringTestBase {
	...
	public function testSingleLink() {
		...
		$this->setExpectedException ( "RuntimeException", "Can't extract from an empty heap" );
		$sl->getNextMerge ();
	}
	...
	public function testDendrogramToClusters() {
		...
		foreach ( $dendrograms as $i => $d ) {
			$this->assertEquals ( $d [1], Hierarchical::dendrogramToClusters ( $d [0], count ( $d [1] ) ), "Error transforming dendrogram $i" );
		}
	}
	public function testClustering1() {
		...
		$hc = new Hierarchical ( new SingleLink (), // use the single link strategy
...
;
		...
		list ( $dendrogram ) = $hc->cluster ( $tset, new DataAsFeatures () );
		$dg = $this->drawDendrogram ( $tset, $dendrogram, 600 ) // width
;
		
		$clusters = Hierarchical::dendrogramToClusters ( $dendrogram, 2 );
		$im = $this->drawClusters ( $tset, $clusters, null, // no centroids
false, // no lines
