...
class WordDocument implements DocumentInterface {
	...
	public function applyTransformation(TransformationInterface $transform) {
		...
		$this->before = array_values ( array_filter ( array_map ( array (
				$transform,
				"transform" 
		), $this->before ), $null_filter ) );
		$this->after = array_values ( array_filter ( array_map ( array (
				$transform,
				"transform" 
		), $this->after ), $null_filter ) );
	}
}
