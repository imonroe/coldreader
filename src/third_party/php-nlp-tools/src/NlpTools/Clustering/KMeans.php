...
class KMeans extends Clusterer {
	 ...
	 *        	The distance metric to be used (Euclidean, Hamming, ...)
	 * @param CentroidFactoryInterface $cf
	 *        	This parameter will be used to create the new centroids from a set of documents
	 * @param float $cutoff
	 *        	When the maximum change of the centroids is smaller than that stop iterating
	 */
	...
	public function cluster(TrainingSet $documents, FeatureFactoryInterface $ff) {
		...
		while ( true ) {
			...
			$distances = array_map ( function ($doc) use(&$centroids, $dist) {
				return array_map ( function ($c) use($dist, $doc) {
					...
					return call_user_func_array ( $dist, array (
							&$c,
							&$doc 
					) );
				}, $centroids );
			...
			$clusters = array_fill_keys ( array_keys ( $centroids ), array () );
			...
			$new_centroids = array_map ( function ($cluster) use(&$docs, $cf) {
				return call_user_func_array ( $cf, array (
						&$docs,
						$cluster 
				) );
			}, $clusters );
			...
			$changes = array_map ( $dist, $new_centroids, $centroids );
