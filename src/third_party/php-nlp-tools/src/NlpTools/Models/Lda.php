...
class Lda {
	 ...
	 *        	The feature factory will be applied to each document and the resulting feature array will be considered as a document for LDA
	 * @param integer $ntopics
	 *        	The number of topics assumed by the model
	 * @param float $a
	 *        	The dirichlet prior assumed for the per document topic distribution
	 * @param float $b
	 *        	The dirichlet prior assumed for the per word topic distribution
	 */
	...
	public function initialize(array &$docs) {
		...
		$this->words_in_doc = array_fill_keys ( $doc_keys, 0 );
		$this->words_in_topic = array_fill_keys ( $topic_keys, 0 );
		$this->count_docs_topics = array_fill_keys ( $doc_keys, array_fill_keys ( $topic_keys, 0 ) );
		$this->count_topics_words = array_fill_keys ( $topic_keys, array () );
		$this->word_doc_assigned_topic = array_fill_keys ( $doc_keys, array () );
		$this->voc = array ();
	 ...
	 *
	 * @param
	 *        	TrainingSet The docs to run lda on
	 * @param $it The
	 ...
	 *
	 * @param $limit_words Limit
	 *        	the results to the top n words
	...
	public function getWordsPerTopicsProbabilities($limit_words = -1) {
		$p_t_w = array_fill_keys ( range ( 0, $this->ntopics - 1 ), array () );
	 ...
	 *
	 * @param $limit_docs Limit
	 *        	the results to the top n docs
	...
	public function getDocumentsPerTopicsProbabilities($limit_docs = -1) {
		$p_t_d = array_fill_keys ( range ( 0, $this->ntopics - 1 ), array () );
	...
	public function getLogLikelihood() {
		...
		foreach ( $this->count_topics_words as $topic => $words ) {
			$lik += $this->log_multi_beta ( $words, $b );
			$lik -= $this->log_multi_beta ( $b, 0, $voccnt );
		}
		foreach ( $this->count_docs_topics as $doc => $topics ) {
			$lik += $this->log_multi_beta ( $topics, $a );
			$lik -= $this->log_multi_beta ( $a, 0, $this->ntopics );
		}
	...
	protected function conditionalDistribution($i, $w) {
		...
		return array_map ( function ($p) use($sum) {
			return $p / $sum;
		}, $p );
	}
	...
	private function log_multi_beta($a, $y = 0, $k = null) {
		if ($k == null) {
			$ay = array_map ( function ($x) use($y) {
				return $x + $y;
			}, $a );
			
			return array_sum ( $this->log_gamma_array ( $ay ) ) - $this->log_gamma ( array_sum ( $ay ) );
		} else {
