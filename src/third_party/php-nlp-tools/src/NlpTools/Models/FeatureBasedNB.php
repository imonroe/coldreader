...
class FeatureBasedNB implements MultinomialNBModelInterface {
	 ...
	 *
	 * @param array $train_ctx
	 *        	The previous training context
	 * @param FeatureFactoryInterface $ff
	 *        	A feature factory to compute features from a training document
	 * @param
	 *        	TrainingSet The training set
	 * @param integer $a_smoothing
	 *        	The parameter for additive smoothing. Defaults to add-one smoothing.
	...
	public function train_with_context(array &$train_ctx, FeatureFactoryInterface $ff, TrainingSet $tset, $a_smoothing = 1) {
		$this->countTrainingSet ( $ff, $tset, $train_ctx ['termcount_per_class'], $train_ctx ['termcount'], $train_ctx ['ndocs_per_class'], $train_ctx ['voc'], $train_ctx ['ndocs'] );
		
		$voccount = count ( $train_ctx ['voc'] );
		
		$this->computeProbabilitiesFromCounts ( $tset->getClassSet (), $train_ctx ['termcount_per_class'], $train_ctx ['termcount'], $train_ctx ['ndocs_per_class'], $train_ctx ['ndocs'], $voccount, $a_smoothing );
	 ...
	 *
	 * @param
	 *        	FeatureFactoryInterface A feature factory to compute features from a training document
	 * @param
	 *        	TrainingSet The training set
	 * @param integer $a_smoothing
	 *        	The parameter for additive smoothing. Defaults to add-one smoothing.
	...
	/**
	 * Count all the features for each document.
	 * All parameters are passed
	 * by reference and they are filled in this function. Useful for not
	 * making copies of big arrays.
	 *
	 * @param FeatureFactoryInterface $ff
	 *        	A feature factory to create the features for each document in the set
	 * @param TrainingSet $tset
	 *        	The training set (collection of labeled documents)
	 * @param array $termcount_per_class
	 *        	The count of occurences of each feature in each class
	 * @param array $termcount
	 *        	The total count of occurences of each term
	 * @param array $ndocs_per_class
	 *        	The total number of documents per class
	 * @param array $voc
	 *        	A set of the found features
	 * @param integer $ndocs
	 ...
	 * training set.
	 *
	 * @param array $class_set
	 *        	Just the array that contains the classes
	 * @param array $termcount_per_class
	 *        	The count of occurences of each feature in each class
	 * @param array $termcount
	 *        	The total count of occurences of each term
	 ...
	 * @param integer $voccount
	 *        	The total number of features found
	 * @return void
	 */
