...
class RegexTokenizer implements TokenizerInterface {
	...
	/**
	 * Iteratively run for each pattern.
	 * The tokens resulting from one pattern are
	 ...
	 *
	 * @param
	 *        	array &$str The tokens to be further tokenized
	 */
	protected function split(array &$str, $pattern) {
		$tokens = array ();
		foreach ( $str as $s ) {
			$tokens = array_merge ( $tokens, preg_split ( $pattern, $s, null, PREG_SPLIT_NO_EMPTY ) );
		}
	...
	protected function match(array &$str, $pattern, $keep) {
		...
		foreach ( $str as $s ) {
			...
			$tokens = array_merge ( $tokens, $m [$keep] );
		}
