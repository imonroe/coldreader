...
class LancasterStemmer extends Stemmer {
	 ...
	 * 
	 * @param array $ruleSet
	 *        	the set of rules that will be used by the lancaster algorithm. if empty
	...
	protected function indexRules(array $rules) {
		...
		foreach ( $rules as $rule ) {
			if (isset ( $this->indexedRules [$rule [self::LOOKUP_CHAR]] )) {
				...
				$this->indexedRules [$rule [self::LOOKUP_CHAR]] = array (
						$rule 
				);
			}
		}
	...
	public function stem($word) {
		...
		do {
			...
			foreach ( $this->indexedRules [$lookupChar] as $rule ) {
				if (strrpos ( $word, substr ( $rule [self::ENDING_STRING], - 1 ) ) === (strlen ( $word ) - strlen ( $rule [self::ENDING_STRING] ))) {
					
					if (! empty ( $rule [self::INTACT_FLAG] )) {
						
						if ($this->originalToken == $word && $this->isAcceptable ( $word, ( int ) $rule [self::REMOVE_TOTAL] )) {
	 ...
	 * 
	 * @param string $word
	 *        	word the rule is being applied on
	 * @param array $rule
	 *        	An associative array containing all the data elements for applying to the word
	 */
	 ...
	 *        	The number of characters to remove from the suffix
	...
	protected function isAcceptable($word, $removeTotal) {
		...
		if ($this->vowelChecker->isVowel ( $word, 0 ) && $length >= 2) {
			return true;
		} elseif ($length >= 3 && ($this->vowelChecker->isVowel ( $word, 1 ) || $this->vowelChecker->isVowel ( $word, 2 ))) {
			return true;
		}
