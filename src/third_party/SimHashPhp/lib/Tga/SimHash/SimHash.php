...
class SimHash {
	...
	protected function findTokenizer($element, $size) {
		...
		if (! $tokenizer) {
			...
			throw new \RuntimeException ( sprintf ( 'No tokenizer is able to tokenize "%s" in %s bits (available tokenizers: %s)', is_object ( $element ) ? 'instance of ' . get_class ( $element ) : gettype ( $element ), $size, implode ( ', ', $availableTokenizers ) ) );
		}
