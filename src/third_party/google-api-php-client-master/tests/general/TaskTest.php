...
class TaskTest extends PHPUnit_Framework_TestCase {
	...
	public function testCustomRestRetryMapReplacesDefaults($errorCode, $errorBody = '{}') {
		$this->client->setClassConfig ( 'Google_Service_Exception', array (
				'retry_map' => array () 
		) );
	...
	public function testCustomRestRetryMapAddsNewHandlers() {
		$this->client->setClassConfig ( 'Google_Service_Exception', array (
				'retry_map' => array (
						'403' => Google_Config::TASK_RETRY_ALWAYS 
				) 
		) );
	...
	public function testRestTimeouts($config, $minTime) {
		...
		$this->assertTaskTimeGreaterThanOrEqual ( $minTime, array (
				$this,
				'makeRequest' 
	...
	public function testCustomCurlRetryMapReplacesDefaults($errorCode, $errorMessage = '') {
		$this->client->setClassConfig ( 'Google_IO_Exception', array (
				'retry_map' => array () 
		) );
	...
	public function testBadTaskConfig($config, $message) {
		...
		new Google_Task_Runner ( $this->client, '', array (
				$this,
				'testBadTaskConfig' 
		) );
	}
	...
	public function testTaskRetryOffByDefault() {
		$this->setNextTaskAllowedRetries ( Google_Config::TASK_RETRY_ALWAYS )->runTask ();
	}
	...
	public function testOneTaskRetryWithError() {
		...
		$this->setNextTasksAllowedRetries ( 2, Google_Config::TASK_RETRY_ALWAYS )->runTask ();
	}
	...
	public function testMultipleTaskRetriesWithErrors() {
		...
		$this->setNextTasksAllowedRetries ( 6, Google_Config::TASK_RETRY_ALWAYS )->runTask ();
	}
	public function testOneTaskRetryWithSuccess() {
		$this->setTaskConfig ( array (
				'retries' => 1 
		) );
		$result = $this->setNextTaskAllowedRetries ( Google_Config::TASK_RETRY_ALWAYS )->setNextTaskReturnValue ( 'success' )->runTask ();
	...
	public function testMultipleTaskRetriesWithSuccess() {
		...
		$result = $this->setNextTasksAllowedRetries ( 2, Google_Config::TASK_RETRY_ALWAYS )->setNextTaskReturnValue ( 'success' )->runTask ();
	...
	public function testTaskWithManualRetries() {
		...
		$task = new Google_Task_Runner ( $this->client, '', array (
				$this,
				'runNextTask' 
		) );
	...
	public function customLimitsProvider() {
		return array (
				array (
						Google_Config::TASK_RETRY_NEVER 
				),
				array (
						Google_Config::TASK_RETRY_ONCE 
				) 
		);
	}
...
;
	 ...
	 *        	The expected minimum execution time
	 ...
	 *
	 * @param array $config
	 *        	The task runner configurations
	 */
	...
	private function setNextResponseThrows($message, $code) {
		...
		$this->mockedCalls [$this->mockedCallsCount ++] = new Google_IO_Exception ( $message, $code, null, $map );
	...
	private function makeRequest() {
		...
		$io = $this->getMockBuilder ( 'Google_IO_Abstract' )->disableOriginalConstructor ()->setMethods ( array (
				'makeRequest' 
		) )->getMockForAbstractClass ();
		
		$io->expects ( $this->exactly ( $this->mockedCallsCount ) )->method ( 'makeRequest' )->will ( $this->returnCallback ( array (
				$this,
				'getNextMockedCall' 
		) ) );
	 ...
	 *
	 * @param boolean $allowedRetries
	 *        	The next `allowedRetries()` return value.
	 *        	
	 * @return TaskTest
	 */
	 ...
	 *
	 * @param integer $count
	 *        	The number of `allowedRetries()` return values.
	 * @param boolean $allowedRetries
	 *        	The `allowedRetries()` return value.
	 *        	
	 * @return TaskTest
	 */
	...
	private function runTask() {
		...
		$exception = $this->getMockBuilder ( 'Google_IO_Exception' )->disableOriginalConstructor ()->setMethods ( array (
				'allowedRetries' 
		) )->getMock ();
		$exceptionCount = 0;
