...
class IoTest extends BaseTest {
	...
	public function testCacheRevalidate() {
		...
		$io = $this->getMockBuilder ( 'Google_IO_Abstract' )->setConstructorArgs ( array (
				$client 
		) )->setMethods ( array (
				'getCachedRequest',
		...
		$io->expects ( $this->once () )->method ( 'getCachedRequest' )->will ( $this->returnValue ( $req ) );
		
		$io->expects ( $this->once () )->method ( 'checkMustRevalidateCachedRequest' )->will ( $this->returnValue ( true ) );
		
		$io->expects ( $this->once () )->method ( 'executeRequest' )->will ( $this->returnValue ( array (
				'{"a": "foo"}',
				array (
						'te' => 'gzip',
