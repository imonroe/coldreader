...
class Url {
	 ...
	 * @static
	 *
	 * @param mixed $item_url
	 *        	Unknown url (can be relative or not)
	...
	public function filterPath($path, $charUnreserved = 'a-zA-Z0-9_\-\.~', $charSubDelims = '!\$&\'\(\)\*\+,;=') {
		return preg_replace_callback ( '/(?:[^' . $charUnreserved . $charSubDelims . ':@\/%]+|%(?![A-Fa-f0-9]{2}))/', function (array $matches) {
	 ...
	 *
	 * @param string $suffix
	 *        	Suffix to add when there is a scheme
	 *        	
	 * @return string
	 */
	 ...
	 *
	 * @param string $prefix
	 *        	Prefix to add when there is a port
	 *        	
	 * @return string
	 */
